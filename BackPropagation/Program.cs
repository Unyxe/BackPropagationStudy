...
namespace BackPropagation
{
    class Program
    {
        ...
        static double[][] data_set = new double[10000][];
        static void Main(string[] args)
        {
            for(int i =0; i < data_set.Length; i++)
            {
                ...
                int b = rn.Next() % 21 - 10;
                int res = a + b;
                data_set[i] = new double[3] {a, b, res };
            }

            //train();
            NeuralNetwork nn = new NeuralNetwork(2, 3 ,2, 1, rn.Next(), false);
            nn.Train(data_set, 10000000);

            while (true)
            {
                Console.WriteLine(nn.CalculateOutput(new double[] { Int32.Parse(Console.ReadLine()), Int32.Parse(Console.ReadLine()) })[0]);
            }
            Console.ReadLine();
        }
